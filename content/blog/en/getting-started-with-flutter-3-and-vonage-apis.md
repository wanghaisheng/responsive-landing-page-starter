...
By the nature of Flutter the code can easily be broken down into three areas, the Flutter code which is writen in Dart, the native Android code which is written in Kotlin and the native iOS code that is written in Swift.

### Flutter

Lets start with the flutter specific code, replace the content of `lib/main.dart` with the below code:
...
This is the complete class needed to build the apps UI and trigger the platform specific methods which we will write in a moment. Lets break down whats going on in each of the methods in this class.
...
This `CallWidget` extends the `StatefulWidget` taking the title and initlising the `CallWidgetState`.
